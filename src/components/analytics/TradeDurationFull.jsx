...
export default function TradeDurationFull({ trades, onDrillDown }) {
  const analysis = useMemo(() => {
    ...
    const avgDuration = closedTrades.length > 0 ? totalMinutes / closedTrades.length : 0;
    ...
    const buckets = bucketDefs.map(def => {
      const bucketTrades = closedTrades.filter(t => t.actual_duration_minutes >= def.min && t.actual_duration_minutes < def.max);
      const wins = bucketTrades.filter(t => (t.pnl_usd || 0) > 0).length;
      const totalPnl = bucketTrades.reduce((sum, t) => sum + (t.pnl_usd || 0), 0);
  ...
  return (
    <div className="backdrop-blur-md bg-gradient-to-br from-[#1a1a1a]/90 to-[#0d0d0d]/90 rounded-xl border border-[#2a2a2a]/50 p-6 mb-6 col-span-2">
      ...
      <div className="grid grid-cols-6 gap-3">
        {analysis.buckets.map((bucket) => {
          ...
          return (
            <div 
              key={bucket.name}
              onClick={() => !isEmpty && bucket.trades.length > 0 && onDrillDown(`Duration: ${bucket.name}`, bucket.trades)}
              className={cn(
                "relative rounded-xl border-2 p-4 overflow-hidden transition-all",
                isEmpty 
                  ? "bg-[#0d0d0d]/50 border-[#2a2a2a]/30 opacity-40" 
                  : isProfit 
                    ? "bg-gradient-to-br from-emerald-500/15 via-[#0d0d0d] to-emerald-500/5 border-emerald-500/40 hover:border-emerald-500/60 hover:scale-[1.02] cursor-pointer"
                    : "bg-gradient-to-br from-red-500/15 via-[#0d0d0d] to-red-500/5 border-red-500/40 hover:border-red-500/60 hover:scale-[1.02] cursor-pointer"
              )}
            >
