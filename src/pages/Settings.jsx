          ...
          {profiles.map(profile => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-emerald-500/20 to-green-500/20 flex items-center justify-center overflow-hidden">
                    {profile.profile_image ? (
                      <img src={profile.profile_image} alt="" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 text-[#666] text-xs">
                      <span>{lang === 'ru' ? 'Сделок' : 'Trades'}: {stats.totalTrades}</span>
                      <span className={cn(
                        stats.totalPnl >= 0 ? "text-emerald-400" : "text-red-400"
                      )}>
                        PNL: {stats.totalPnl >= 0 ? '+' : ''}{stats.totalPnl.toFixed(0)}$
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {profiles.length > 1 && !profile.is_active && (
                  <Button
                    ...
                    className="w-full border-red-500/30 text-red-400 hover:bg-red-500/10 h-7 text-xs"
                  >
                    {lang === 'ru' ? 'Удалить' : 'Delete'}
                  </Button>
                )}
